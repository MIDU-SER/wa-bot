...
async function whatsAsena () {
    ...
    conn.browserDescription = ["wa-Bot v1.0", "Safari", '1.0.0']
...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp...')}`);
    ...
    conn.on('open', async () => {
        ...
        console.log(
            chalk.green.bold('VavaBot Working Fine 𝚗𝚘𝚠 🍃'));
                ...
                if ((command.on !== undefined && (command.on === 'image' || command.on === 'photo')
                    ...
                    if (sendMsg) {
                        ...
                        try {
                            ...
                            if (config.LANG == 'ML') {
                                await conn.sendMessage(conn.user.jid, '*~_________~ Vava ~______~*' +
                                    '\n```🌀 Entho Oru Preshnam Ond```' +
                                    '\n\n*⚠️ ' + error + '*\n'
                                    , MessageType.text);
                            } else {
                                await conn.sendMessage(conn.user.jid, '*~_________~ Vava ~______~*' +
